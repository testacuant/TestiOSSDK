...
import AcuantCamera
import AcuantIPLiveliness
import AcuantCommon
import AcuantDocumentProcessing
import AcuantFaceMatch
import AcuantHGLiveliness

class RootViewController: UIViewController , InitializationDelegate,CreateInstanceDelegate,UploadImageDelegate,GetDataDelegate, FacialMatchDelegate,DeleteDelegate,LivelinessTestDelegate,AcuantHGLivelinessDelegate,CameraCaptureDelegate{
    ...
    func showFacialCaptureInterface(){
        ...
        AcuantIPLiveliness.showLiveFaceCaptureInterface(del: self)
    ...
    public func setCapturedImage(image:Image? , barcodeString:String?){
        ...
        DispatchQueue.global().async {
            ...
            DispatchQueue.main.async {
                self.vcUtil.hideActivityIndicator(uiView: self.view)
                if(croppedImage?.error != nil && image?.error?.errorCode != AcuantErrorCodes.ERROR_LowResolutionImage){
