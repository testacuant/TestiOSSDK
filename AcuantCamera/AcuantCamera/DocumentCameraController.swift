...
import AcuantCommon
public class DocumentCameraController : UIViewController,DocumentCaptureDelegate{
    ...
    func startCameraView() {
        let captureDevice: AVCaptureDevice = AVCaptureDevice.default(.builtInWideAngleCamera, for: AVMediaType.video, position: .back)!
            ...
            self.captureSession = DocumentCaptureSession.getDocumentCaptureSession(delegate: self,captureDevice: captureDevice)
    ...
    public func documentCaptured(image: Image?, barcodeString: String?) {
        if((image?.error == nil || image?.error?.errorCode==AcuantErrorCodes.ERROR_LowResolutionImage)  && image?.image != nil){
            self.messageLayer.backgroundColor = UIColor.green.cgColor
            self.messageLayer.fontSize = 30
            self.messageLayer.string = "SUCCESS"
            self.navigationController?.popViewController(animated: true)
            self.cameraCaptureDelegate?.setCapturedImage(image: image, barcodeString: barcodeString)
        }else{
            let resolutionThreshold = CaptureConstants.RESOLUTION_THRESHOLD
            if(image?.error?.errorCode == AcuantErrorCodes.ERROR_CouldNotCrop || (image?.dpi)! < CaptureConstants.NO_DOCUMENT_DPI_THRESHOLD ){
                self.messageLayer.backgroundColor = UIColor.gray.cgColor
                animateMessage(message: "ALIGN")
            }else if(image?.error?.errorCode == AcuantErrorCodes.ERROR_LowResolutionImage && (image?.dpi)! < resolutionThreshold){
                self.messageLayer.backgroundColor = UIColor.gray.cgColor
                animateMessage(message: "MOVE CLOSER")
            }else if(image?.isCorrectAspectRatio == false){
                self.messageLayer.backgroundColor = UIColor.gray.cgColor
                animateMessage(message: "ALIGN")
            }else{
    ...
    public func documentCapturedWithError() {
        self.navigationController?.popViewController(animated: true)
        self.cameraCaptureDelegate?.setCapturedImage(image: nil, barcodeString: nil)
    }
    
    public func readyToCapture(){
        self.messageLayer.backgroundColor = UIColor.red.cgColor
        self.messageLayer.fontSize = 30
        self.messageLayer.string = "HOLD STEADY"
        captured = true
    }
